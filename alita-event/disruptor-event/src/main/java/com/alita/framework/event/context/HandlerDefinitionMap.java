...
public class HandlerDefinitionMap implements Map<String, HandlerDefinitionMap.Section> {

    private static final Logger LOG = LoggerFactory.getLogger(HandlerDefinitionMap.class);
    ...
    /**
     * Creates a new empty {@code HandlerDefinitionMap} instance.
     */
    public HandlerDefinitionMap() {
        this.sections = new LinkedHashMap<>(16);
    }

    /**
     * Creates a new {@code HandlerDefinitionMap} instance with the specified defaults.
     *
     * @param defaults the default sections and/or key-value pairs to copy into the new instance.
     */
    public HandlerDefinitionMap(HandlerDefinitionMap defaults) {
        this();
        if (defaults == null) {
    ...
    /**
     * Returns {@code true} if no sections have been configured, or if there are sections,
     * but the sections themselves are all empty, {@code false} otherwise.
     */
    ...
    /**
     * Returns the names of all sections managed by this {@code HandlerDefinitionMap} instance or an empty collection
    ...
    /**
     * Returns the sections managed by this {@code HandlerDefinitionMap} instance or an empty collection if there are
     * no sections.
     */
    ...
    private static String cleanName(String sectionName) {
        ...
        if (name == null) {
            LOG.debug("Specified name was null or empty.  Defaulting to the default section (name = \"\")");
     ...
     * If the section does not yet exist, it will be created.
     * If the {@code sectionName} is null or empty, the name/value pair will be placed in the
     ...
     * @throws EventHandleException if unable
     */
    public void load(InputStream is) throws EventHandleException {
        if (is == null) {
     ...
     * This implementation will close the reader after it has finished loading.
     *
    ...
    public void load(Reader reader) {
        ...
        try {
            ...
            try {
                ...
                LOG.error("Unable to cleanly close the InputStream scanner.  Non-critical - ignoring.", e);
            }
        }
    }
    ...
    public boolean equals(Object obj) {
        if (obj instanceof HandlerDefinitionMap) {
            HandlerDefinitionMap handlerDefinitionMap = (HandlerDefinitionMap) obj;
            return this.sections.equals(handlerDefinitionMap.sections);
        }
        return false;
    }
    ...
    public String toString() {
        if (this.sections == null || this.sections.isEmpty()) {
            return "<empty HandlerDefinitionMap>";
        } else {
    ...
    /**
     * An {@code HandlerDefinitionMap.Section} is String-key-to-String-value Map, identifiable by a
     * {@link #getName() name} unique within an {@link HandlerDefinitionMap} instance.
     */
    public static class Section implements Map<String, String> {
        ...
        protected static String[] splitKeyValue(String keyValueLine) {
            ...
            for (int i = 0; i < line.length(); i++) {
                ...
                if (buildingKey) {
                    ...
                    boolean flag = valueBuffer.length() == 0 && isKeyValueSeparatorChar(c) && !isCharEscaped(line, i);
                    if (!flag) {
            ...
            LOG.debug("Discovered key/value pair: {}={}", key, value);
        ...
        private static Map<String, String> toMapProps(String content) {
            ...
            while (scanner.hasNextLine()) {
                ...
                if (ArrayUtils.isNotEmpty(kvPair)) {
