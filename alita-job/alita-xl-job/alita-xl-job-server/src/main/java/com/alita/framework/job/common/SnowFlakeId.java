 ...
 * </p>
 * <ul>
 * <li>48, 64 or 128-bit integers.</li>
 * <li>Distributed and unique across all nodes (provided each node has a unique
 * id).</li>
 * <li>Ascending: within a node, {@code next_id > previous_id}.</li>
 ...
 * <li>Non-serial: {@code next_id} may NOT be {@code previous_id + 1}.</li>
 * </ul>
 *
...
public class SnowFlakeId {
    ...
    private final static LoadingCache<Long, SnowFlakeId> idGenerators = CacheBuilder
            .newBuilder().expireAfterAccess(3600, TimeUnit.SECONDS)
            .removalListener(new RemovalListener<Long, SnowFlakeId>() {
                @Override
                public void onRemoval(RemovalNotification<Long, SnowFlakeId> entry) {
                    entry.getValue().destroy();
                }
            }).build(new CacheLoader<Long, SnowFlakeId>() {
                @Override
                public SnowFlakeId load(Long nodeId) throws Exception {
                    SnowFlakeId idGen = new SnowFlakeId(nodeId);
                    idGen.init();
                    return idGen;
                }
            });
    ...
    /**
     * Gets an {@link SnowFlakeId} instance.
     *
     * @return
     */
    ...
    /**
     * Gets an {@link SnowFlakeId} instance for a node.
     *
     * @param nodeId
     * @return
     */
    public static SnowFlakeId getInstance(long nodeId) {
        try {
    ...
    /**
     * Constructs a new {@link SnowFlakeId} instance with specified
     * node id.
     *
     * @param nodeId
     */
    protected SnowFlakeId(long nodeId) {
        this.nodeId = nodeId;
    }
     ...
     *
     * Format: {@code <41-bit: timestamp><0 or 16bit:sequence number>}
     *
     ...
     *
     * Format: {@code <32-bit: timestamp><3-bit: node id><13 bit: sequence number>}
     *
     ...
     *
     * Format: {@code <41-bit: timestamp><0-bit: node id><7 bit: sequence number>}
     *
     * Where timestamp is in millisec, minus the epoch.
     *
     * @return
     ...
     *
     * Format: {@code <41-bit: timestamp><10-bit: node id><13 bit: sequence number>}
     *
     * Where timestamp is in millisec, minus the epoch.
     *
     * @return
     ...
     *
     * Format: {@code <64-bit: timestamp><48-bit: node id><16 bit: sequence number>}
     *
